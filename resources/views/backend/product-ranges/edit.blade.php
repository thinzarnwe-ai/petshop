        ...
        <div class="card my_card">
            <div class="card-header bg-transparent">
                <a href="{{route('product.range')}}" class="card-title mb-0 d-inline-flex align-items-center create_title">
                    ...
                    <span class="create_sub_title">Range ကိုပြုပြင်မည်</span>
                </a>
            </div><!-- end card header -->
            <div class="card-body">
                <div class="row d-flex justify-content-center">
                    <div class="col-xl-9">
                        <form method="POST" action="{{ route('product.range.update', $productRange->id) }}" id="category_create" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                                <label for="employeeName" class="form-label mb-3">အမည်</label>
                                <input type="text" class="form-control" name="name" value="{{$productRange->name}}">
                            </div>
